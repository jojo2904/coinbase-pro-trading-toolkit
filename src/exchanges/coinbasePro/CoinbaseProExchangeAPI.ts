...
import { AuthCallOptions, AuthHeaders, CoinbaseProAuthConfig, CoinbaseProConfig, CoinbaseProHTTPError, OrderbookEndpointParams } from './CoinbaseProInterfaces';
import { Account,
         AuthenticatedClient,
         CoinbaseAccount,
         OrderInfo,
         ...
         ProductInfo,
         ProductTicker,
         PublicClient } from 'coinbase-pro';
import { APIError, extractResponse, CBPTTError, HTTPError } from '../../lib/errors';
import request = require('superagent');
import querystring = require('querystring');
import crypto = require('crypto');
import Response = request.Response;

export const COINBASE_PRO_API_URL = 'https://api.pro.coinbase.com';
...
export class CoinbaseProExchangeAPI implements PublicExchangeAPI, AuthenticatedExchangeAPI, ExchangeTransferAPI {
    ...
    private readonly auth: CoinbaseProAuthConfig;
    private readonly logger: Logger;

    constructor(options: CoinbaseProConfig) {
        this.owner = 'Coinbase Pro';
        this._apiURL = options.apiUrl || COINBASE_PRO_API_URL;
    ...
    loadProducts(): Promise<Product[]> {
        return this.publicClient.getProducts()
            ...
            }).catch((err: CoinbaseProHTTPError) => {
                return Promise.reject(new HTTPError('Error loading products from Coinbase Pro', extractResponse(err.response)));
            });
    }
        ...
        return this.publicClient.getProductOrderBook(product, params)
            .then((orders) => {
                if (!(orders.bids && orders.asks)) {
                    return Promise.reject(new HTTPError(`Error loading ${product} orderbook from Coinbase Pro`, {status: 200, body: orders}));
                }
                return orders;
            }).catch((err: CoinbaseProHTTPError) => {
                return Promise.reject(new HTTPError(`Error loading ${product} orderbook from Coinbase Pro`, extractResponse(err.response)));
            });
    }
    ...
    loadTicker(product: string): Promise<Ticker> {
        return this.publicClient.getProductTicker(product)
            .then((ticker: ProductTicker) => {
                ...
                return Promise.reject(new HTTPError(`Error loading ${product} ticker from Coinbase Pro`, extractResponse(err.response)));
            });
    }
    ...
    loadCandles(options: CandleRequestOptions): Promise<Candle[]> {
        ...
        return this.publicClient.getProductHistoricRates(product, {
            ...
            return Promise.reject(new HTTPError(`Error loading ${product} candles from Coinbase Pro`, extractResponse(err.response)));
        });
    }
    ...
    placeOrder(order: PlaceOrderMessage): Promise<LiveOrder> {
        if (!this.authClient) {
            return Promise.reject(new CBPTTError('No authentication details were given for this API'));
        }
        let coinbaseProOrder: OrderParams;
        switch (order.orderType) {
            ...
            case 'stop':
                coinbaseProOrder = {
                    ...
                    client_oid: order.clientId,
                    stp: order.extra && order.extra.stp
                } as OrderParams; // Override for incomplete definition in Coinbase Pro lib
                break;
            default:
                return Promise.reject(new CBPTTError(`Invalid order type: ${order.type}`));
        }

        return this.authClient.placeOrder(coinbaseProOrder).then((result: OrderResult) => {
            // Check for error
    ...
    loadOrder(id: string): Promise<LiveOrder> {
        ...
        return this.authClient.getOrder(id).then((order: OrderInfo) => {
            ...
            return Promise.reject(new HTTPError('Error loading order details on Coinbase Pro', extractResponse(err.response)));
        });
    }
    ...
    loadAllOrders(product?: string): Promise<LiveOrder[]> {
        ...
        const loop: (after?: string) => Promise<LiveOrder[]> = (after?: string) => {
            return self.loadNextOrders(product, after).then((result) => {
                const liveOrders: LiveOrder[] = result.orders.map(CoinbaseProOrderInfoToOrder);
    ...
    loadBalances(): Promise<Balances> {
        ...
        return this.authClient.getAccounts().then((accounts: Account[]) => {
            ...
            return Promise.reject(new HTTPError('Error loading account balances on Coinbase Pro', extractResponse(err.response)));
        });
    }
    ...
    handleResponse<T>(req: Promise<Response>, _meta: any): Promise<T> {
        return req.then((res: Response) => {
            ...
            const err: HTTPError = new HTTPError(`Error handling Coinbase Pro request for ${(req as any).url}`, res);
            return Promise.reject(err);
        }).catch((err) => {
            const error: CBPTTError = new CBPTTError('A Coinbase Pro API request failed.', err); // TODO add req url here
    ...
    requestCryptoAddress(cur: string): Promise<CryptoAddress> {
        return this.loadCoinbaseAccount(cur, false).then((account: CoinbaseAccount) => {
            ...
            if (!id) {
                return Promise.reject(new CBPTTError('Coinbase account does not have an ID'));
            }
            ...
            if (!validResult) {
                return Promise.reject(new CBPTTError(`Could not obtain a valid crypto address for${cur}`));
            }
    ...
    requestWithdrawal(req: WithdrawalRequest): Promise<TransferResult> {
        ...
        return this.authClient.withdrawCrypto(params).catch((err: CoinbaseProHTTPError) => {
            return Promise.reject(new HTTPError(`Error on Coinbase Pro withdrawal request`, extractResponse(err.response)));
        });
    }
    ...
    loadCoinbaseAccount(currency: string, force: boolean): Promise<CoinbaseAccount> {
        return this.loadCoinbaseAccounts(force).then((accounts: CoinbaseAccount[]) => {
            ...
            return Promise.reject(new CBPTTError(`No Coinbase account for ${currency} exists`));
        });
    }
...
function CoinbaseProOrderResultToOrder(order: OrderResult): LiveOrder {
    let size: BigJS;
    let price: BigJS;
    if (+order.size > 0) {
        size = Big(order.size);
...
function CoinbaseProOrderInfoToOrder(order: OrderInfo): LiveOrder {
    let size: BigJS;
    let price: BigJS;
    if (+order.size > 0) {
        size = Big(order.size);
